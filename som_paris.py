...
som_paris = pd.read_csv("SOM_paris_ldi_LF11.tsv", sep='\t')

#som_paris.head()
#print(som_paris[['H']])

som_paris["molecular_formula"] = "C" + (som_paris["C"]).astype(str) + "H" + (som_paris["H"]).astype(str) + "N" + (som_paris["N"]).astype(str) + "O" + (som_paris["O"]).astype(str) + "S" + (som_paris["S"]).astype(str)

som_paris["abundance_int"] = som_paris["I"]
som_paris["mz_theo_neutral"] = som_paris["mz"]
som_paris["H/C"] = som_paris["H"]/som_paris["C"] 
som_paris["N/C"] = som_paris["N"]/som_paris["C"]
som_paris["O/C"] = som_paris["O"]/som_paris["C"]

#print(som_paris[['abundance_int']])
#print(som_paris[['id']])

som_paris_2 = som_paris[['id','mz_theo_neutral','abundance_int', 'C', 'H', 'N', 'O', 'S', 'molecular_formula', 'Sample', 'H/C', 'O/C', 'N/C']].copy()

#som_paris_2

som_paris_2.to_csv('edited_som_paris.tsv', sep="\t", index=False)  
...
plt.savefig('Figure1_SOM_PARIS'+name+'_Comp'+str(FILTER_COMPONENTS)+'    '+str(color_type)+'    network'+'.png')
plt.show()
...
plt.savefig('SupportingFigure1_SOM_PARIS-Frequency_Histogram'+'.png')
plt.show()
...
##    plt.colorbar(nc)
   ...
   nc.figure.axes[1].tick_params(axis="y", labelsize=21)              ## change the tick label size of colorbar
   plt.axis('off')
   plt.savefig('SupportingFigure4_SOM_PARIS_' + color_type +'.png')
   plt.show()
...
#trafos          =       ['O','CO','NH3'] # Minimal Set - H2, 314 components.

#trafos          =       ['H2','CO','NH3'] # Minimal Set - O, 290 components.

#trafos          =       ['H2','O','NH3'] # Minimal Set - CO, 286 components.

trafos          =       ['H2','O','CO'] # Minimal Set - NH3, 282 components.
...
##    plt.colorbar(nc)
   ...
   nc.figure.axes[1].tick_params(axis="y", labelsize=21)              ## change the tick label size of colorbar
   plt.axis('off')
   plt.savefig('SupportingFigure7_SOM_PARIS_ ' + color_type + '_minimal_set_NH3' + '.png')
   plt.show()
...
#
    ...
    print('\n\n\n\n')
    #
    FILTER_COMPONENTS  = [41.0]      #  list(df_comp_freq_comb.index)[:nComponents]
    xEDGES = edges3[(edges3['component source'].isin(FILTER_COMPONENTS))]
    xNODES = xNODES0[(xNODES0['component'].isin(FILTER_COMPONENTS))]
    G = nx.from_pandas_edgelist(xEDGES, 'Source', 'Target', create_using=nx.Graph())
    a = pd.DataFrame(G.nodes(data = True)).rename(columns={0:'id'}).drop([1], axis=1)
...
plt.savefig('SupportingFigure2_SOM_PARIS_Component41.0_DBE_NC' + '.png')
...
plt.savefig('SupportingFigure2_SOM_PARIS_Component41.0_DBE_OC' + '.png')
#plt.savefig('SupportingFigure2_SOM_ICE_Component6.0_DBE_NC' + '.png')
plt.show()
